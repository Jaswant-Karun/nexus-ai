...
export default function ChatHistoryPage() {
  ...
  useEffect(() => {
    fetch('/api/conversations')
      ...
      .then((payload: { data?: Conversation[] }) => setThreads(payload.data ?? []))
      .catch(() => setThreads([]));
  }, []);

  const filtered = threads.filter(t => 
    t.title.toLowerCase().includes(search.toLowerCase()) ||
    (t.messages[0]?.content ?? '').toLowerCase().includes(search.toLowerCase())
  );
          ...
          {filtered.map(thread => (
            ...
            >
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-3 pt-1 text-[11px] text-slate-500">
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" /> {new Date(thread.lastMessageAt).toLocaleString()}
                  </span>
                  <span>•</span>
              ...
              <div className="flex items-center gap-2 self-end sm:self-center">
                <Link
                  href={`/chat?conversation=${thread.id}`}
