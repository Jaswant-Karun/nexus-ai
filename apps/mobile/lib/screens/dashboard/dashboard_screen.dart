...
class _DashboardScreenState extends State<DashboardScreen> {
			...
			child: Scaffold(
				...
				bottomNavigationBar: NavigationBar(
					...
					destinations: const [
						...
						NavigationDestination(icon: Icon(Icons.bolt_outlined), selectedIcon: Icon(Icons.bolt), label: 'Activity'),
	...
	Widget _welcomeCard() {
		return Container(
			...
			decoration: BoxDecoration(
				...
				gradient: const LinearGradient(colors: [Color(0xff2457c5), Color(0xff4389ed)]),
			),
			child: Row(
				children: [
					const Expanded(
						child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
							Text('Build something\nremarkable today.', style: TextStyle(color: Colors.white, fontSize: 23, fontWeight: FontWeight.w700, height: 1.15)),
							SizedBox(height: 9),
							Text('Your AI workspace is ready when you are.', style: TextStyle(color: Color(0xffdce8ff), fontSize: 13)),
						]),
					),
					Container(
						width: 66,
	...
	Widget _metricCard(String value, String label, IconData icon, Color color) {
		return Expanded(
			child: Container(
				...
				child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
					Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: _blue, size: 19)),
		...
		return Expanded(
			child: InkWell(
				...
				child: Container(
					...
					child: Column(children: [Icon(icon, color: color, size: 23), const SizedBox(height: 8), Text(label, textAlign: TextAlign.center, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600))]),
				),
			),
		);
	}
	...
	Widget _recentActivity() {
		return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
			...
			_activityTile(Icons.check_circle_rounded, const Color(0xff16866b), 'Report generated', 'Marketing insights • 12 min ago'),
			_activityTile(Icons.auto_awesome, _blue, 'Nexus completed a task', 'Product roadmap • 1 hour ago'),
			_activityTile(Icons.person_add_alt_1_rounded, const Color(0xff6f52c9), 'New team member joined', 'Design system • Yesterday'),
		]);
	}
	...
	Widget _activityPage() => _simplePage('Activity', Icons.bolt_rounded, 'A clear view of your workspace', ['Report generated', 'Workflow completed', 'Project updated'], const [1, 1, 1]);

	Widget _settingsPage() {
		return ListView(padding: const EdgeInsets.fromLTRB(20, 24, 20, 30), children: [
			...
			_settingTile(Icons.person_outline, 'Profile', 'Alex Morgan', () {}),
	...
	Widget _settingTile(IconData icon, String title, String subtitle, VoidCallback onTap) {
		return ListTile(contentPadding: EdgeInsets.zero, leading: Icon(icon, color: _blue), title: Text(title, style: const TextStyle(fontWeight: FontWeight.w600)), subtitle: Text(subtitle), trailing: const Icon(Icons.chevron_right), onTap: onTap);
	}

	Widget _simplePage(String title, IconData icon, String subtitle, List<String> items, List<double> progress) {
		return ListView(padding: const EdgeInsets.fromLTRB(20, 24, 20, 30), children: [
